...
public class DeclineFriendRequestHandler(IFriendsRepository friendsRepository) : IRequestHandler<DeclineFriendRequestCommand>
{
    ...
    {
        var friendsRequest = await friendsRepository.GetFriendRequests(request.DeclineFriendRequestDto.UserId);
        var requestToDecline = friendsRequest.FirstOrDefault(fr => fr.Id == request.DeclineFriendRequestDto.FriendRequestId);
